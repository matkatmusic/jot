"""RED tests for plate_summaryStop migration.

Tests the per-invocation SessionEnd hook behavior for the spawned plate-summary
agent. Authored from intent + bash docstring (RELAXED_COVERAGE: no paired
bash _tests existed for plate_summary_stop).
"""
import os
import sys
...
import pytest
from _tmp_plate_summaryStop import plate_summaryStop
...
def test_missing_repo_arg_is_noop(tmp_path):
    ...
    out.write_text("body")
    with patch("_tmp_plate_summaryStop.subprocess.run") as run:
        # Test action:

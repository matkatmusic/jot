...
python3 "$CLAUDE_PLUGIN_ROOT/common/scripts/plate/instance_rw.py" create-instance "$INSTANCE" done-smoke "$TMPTEST" main

# Seed plate 1
echo "plate1 work" >> a.txt
STASH1=$(bash "$CLAUDE_PLUGIN_ROOT/skills/plate/scripts/snapshot-stash.sh" done-smoke plate-1)
...
STASH2=$(bash "$CLAUDE_PLUGIN_ROOT/skills/plate/scripts/snapshot-stash.sh" done-smoke plate-2)
...
# Run done.sh
bash "$CLAUDE_PLUGIN_ROOT/skills/plate/scripts/done.sh" done-smoke > "$TMPTEST/done.out" 2>&1
...
python3 "$CLAUDE_PLUGIN_ROOT/common/scripts/plate/instance_rw.py" create-instance "$PARENT_INSTANCE" parent-cascade "$TMPTEST" main
python3 "$CLAUDE_PLUGIN_ROOT/common/scripts/plate/instance_rw.py" create-instance "$CHILD_INSTANCE" child-cascade "$TMPTEST" main
...
bash "$CLAUDE_PLUGIN_ROOT/skills/plate/scripts/done.sh" child-cascade > "$TMPTEST/child-done.out" 2>&1 || true
